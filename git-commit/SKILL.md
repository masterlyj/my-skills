---
name: git-commit
description: Git 提交消息质量规范。用于编写、审查、拆分或校验提交消息、合并请求标题、压缩合并标题和发布变更记录；基于约定式提交，适配中文祈使句、影响范围选择、正文分点、破坏性变更、提交粒度和钩子/流水线自动化校验。禁止添加人工智能署名、工具署名或未经要求的协作者署名。
---
...
写法：摘要和正文之间空一行；每行不超过 72 字符；正文用分点列表（`- ` 开头）逐条列出背景、原因和取舍，每条一行，不用整段散文。
...
### 7.1 同一文件内的改动分属不同提交：`git add -p`

整文件的 `git add` 只能按文件取舍；当同一个文件里的多处改动（hunk）要分别进多个提交时，用 `git add -p <文件>` 按 hunk 暂存：

```bash
git add -p llm_client.py
```
...
| `n` | 跳过当前 hunk |
| `s` | 把当前 hunk 拆得更细 |
| `e` | 手动编辑当前 hunk（`s` 拆不动，或只想保留 hunk 里部分行时用） |
| `q` | 退出，不再处理剩余 hunk |

多个文件共同构成一个逻辑提交时，按提交而不是按文件推进：对每个涉及的文件跑一次 `git add -p`，只选出属于当前提交的 hunk，提交后再处理下一个逻辑提交涉及的文件。

规则：

- 暂存错了用 `git restore --staged <文件>` 清空该文件的暂存区重新选择，不要整个重来。
- 提交之间如果存在依赖（后一个提交引用了前一个才新增的内容），先提交被依赖的一方，否则中间提交无法独立跑通，影响 `git bisect`。
- 拆分只解决暂存边界，提交消息仍按本规范其余章节书写。
